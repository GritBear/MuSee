...
public class BaseGrid {
	...
	protected static final int limit = 65536;
	...
	public BaseGrid(int quadsAcross, int quadsDown, boolean useFixedPoint) {
		final int vertsAcross = quadsAcross * 2;
		final int vertsDown = quadsDown * 2;
		if (vertsAcross < 0 || vertsAcross >= limit) {
			throw new IllegalArgumentException("quadsAcross");
		}
		if (vertsDown < 0 || vertsDown >= limit) {
			throw new IllegalArgumentException("quadsDown");
		}
		if (vertsAcross * vertsDown >= limit) {
			throw new IllegalArgumentException("quadsAcross * quadsDown >= " + limit);
		}
