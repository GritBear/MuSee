...
void MelodyObj::Init(int startingTone, int actualNumberOfExtractionPulse)
{
	...
	endTimeInMS = theSpeedControl->GetTimeVisualSound() * thousand;
	...
	theChildren->SetCleanLim(-2, 4, 2, -2);
...
void MelodyObj::NextAnmiationInfo(){ //default method (deprecated)
	theCreationCore->NextAnimation(targetAnimationIndex, probabilityVect);
	animationIndexAlreadyChose = true;
}

void MelodyObj::NextAnmiationInfo(IndexThemeControl * theIndexStruct){
	targetAnimationIndex = theIndexStruct->animationIndex;
	currentThemeID = theIndexStruct->themeID;
	animationIndexAlreadyChose = true;
}
...
bool MelodyObj::BestMatchParameters(int AnimationIndex){
	...
	//if(theChildren->GetANum()>0)
	//	x1 = theChildren->GetAniList().back()->GetFinalVertex().X;
	//else
...
// tone_rank = 0 means the tone is the highest, 1 means second highest and so on
//------------------------------------------------
void MelodyObj:: AcceptTone(int tone, int rank){
	// first update the times
	if(curToneTimeInMS != NULL){
		previousToneTimeInMS = curToneTimeInMS;
		curToneTimeInMS = theSpeedControl->GetTimeVisualSound() * thousand;
	}else{
		curToneTimeInMS = theSpeedControl->GetTimeVisualSound() * thousand;
	}
